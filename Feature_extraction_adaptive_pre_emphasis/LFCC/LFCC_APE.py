...
def main(
        ...
        output_root: str = "D:/Fish_Feeding_Intensity/lfcc_features_APE",
    ...
    config_name = (
        f"sr_{config.sr}_"
        f"pre_{config.pre_emphasis}_"
        f"frame_{config.frame_length}_"
        f"hop_{config.hop_length}_"
        f"win_{config.windowing}_"
        f"filters_{config.num_filters}_"
        f"lfcc_{config.n_lfcc}_"
        f"std_{config.use_std}_"
        f"deltas_{config.use_deltas}_"
        f"ddeltas_{config.use_double_deltas}_"
        f"{config_hash}"
    )
    ...
    print(f"Workers    : {max_workers or 'all'}")
