...
Enable `feature_selection` to tune an RF or LGBM selector, rank features by `feature_importances_`, keep the top `ratio` features, and then run the normal model fine tuning on the selected feature matrix. Set `strategy` to `global` to select after audio-video concatenation, or `modality` to select each modality separately with the same `ratio` and then concatenate the selected features. The selector first evaluates the default RF/LGBM parameters, then evaluates `n_trials` random parameter sets, so `n_trials: 20` means 21 selector candidates total. `n_trials` and `trial_n_jobs` under `feature_selection` apply only to the selector.

For modality-level selection, use:
...
outputs/
  holdout/
    ...
    run_info.json
    feature_selection/
      global_RF_ratio_0.2_trials_20/
        feature_importances.csv
        selected_features.csv
        selection_info.json
      modality_RF_ratio_0.2_trials_20/
        selection_info.json
        audio/
          feature_importances.csv
          selected_features.csv
