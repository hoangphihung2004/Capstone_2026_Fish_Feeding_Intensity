...
class ViTBase16(BaseBackbone):
    """
    ViT-Base/16 backbone adapted for mono Mel-spectrogram classification.
    Input:  [B, 1, T, M]
    ...
    def __init__(
        ...
        if pretrained:
            self.input_size = weights.meta["min_size"][0]
            self.model = vit_b_16(weights=weights)
        else:
            self.input_size = input_size
            self.model = vit_b_16(weights=None, image_size=input_size)

        self.model.conv_proj = conv3_to_conv1(self.model.conv_proj)

        in_features = self.model.heads.head.in_features
        self.model.heads.head = nn.Linear(in_features, classes_num)
        init_linear(self.model.heads.head)
