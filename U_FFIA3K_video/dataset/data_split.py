...
LABEL_TO_CLASS = {0: "none", 1: "strong", 2: "middle", 3: "weak"}
LABEL_ORDER = [0, 1, 2, 3]
...
class BaseDataSplitter(ABC):
    ...
    def __init__(self, config: SplitterConfig) -> None:
        ...
        if os.path.isdir(video_dir):
            ...
            raise FileNotFoundError(
                f"U_FFIA3K_video splitting requires a 'video' directory inside dataset_path or dataset_path pointing to a video root: '{self.dataset_path}'"
            )
    ...
    def get_file_list(self, split_name: str) -> List[str]:
        ...
        Args:
            split_name (str): The feeding intensity class name to scan.
                Example: 'strong', 'middle', 'weak', 'none'
    ...
    def _validate_split_policy(self, train_dict: List[List], test_dict: List[List], val_dict: List[List]) -> None:
        ...
        for split_name in ["train", "test", "val"]:
            ...
            logger.info(
                f"  - {split_name}: "
                f"none={counts[0]}, strong={counts[1]}, middle={counts[2]}, weak={counts[3]}"
            )
    ...
    def _save_splits(self, train_dict: List[List], test_dict: List[List], val_dict: List[List], output_dir: Path) -> None:
        ...
        for split_name, samples in formatted_splits.items():
            counts = {"none": 0, "strong": 0, "middle": 0, "weak": 0}
            for s in samples:
...
class FishDataSplitter(BaseDataSplitter):
    ...
    def split_data(self) -> Tuple[List[List], List[List], List[List]]:
        ...
        middle_list = self.get_file_list(split_name='middle')
        logger.info(f"Class 'middle': Found {len(middle_list)} files.")
        ...
        def build_entries() -> List[List]:
            return (
                ...
                [self._make_multimodal_entry(video, 2) for video in middle_list] +
        ...
        if self.split_strategy == "time_series":
            ...
            middle_list = sorted(middle_list, key=self._temporal_path_key)
        ...
        if self.split_strategy == "time_series":
            ...
            middle_train = middle_list[:-holdout_size]
            ...
            middle_val = middle_list[-holdout_size:-self.test_sample_per_class]
            ...
            middle_test = middle_list[-self.test_sample_per_class:]
            ...
            middle_val = middle_list[self.test_sample_per_class:2*self.test_sample_per_class]
            ...
            middle_train = middle_list[2*self.test_sample_per_class:]
        ...
        for class_name, size in [('strong', len(strong_list)), ('middle', len(middle_list)), ('weak', len(weak_list)), ('none', len(none_list))]:
        ...
        logger.info(f"       - class 'middle': Train={len(middle_train)}, Test={len(middle_test)}, Val={len(middle_val)}")
        ...
        train_dict = (
            ...
            [self._make_multimodal_entry(video, 2) for video in middle_train] +
        ...
        test_dict = (
            ...
            [self._make_multimodal_entry(video, 2) for video in middle_test] +
        ...
        val_dict = (
            ...
            [self._make_multimodal_entry(video, 2) for video in middle_val] +
