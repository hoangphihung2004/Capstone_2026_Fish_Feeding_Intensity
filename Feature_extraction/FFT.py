...
def process_single_file(row, dataset_dir, extractor_config):
    ...
    try:
        extractor = FFT(extractor_config)
...
def main(
    ...
    print(f"Executing parallel FFT extraction using {max_workers or 'all'} CPU cores...")
    ...
    with output_csv_path.open("w", encoding="utf-8", newline="") as output_file:
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in tqdm(as_completed(futures), total=len(futures), desc="Extracting FFT features", unit="file"):
