...
def _log_run_configuration(cfg, root: Path) -> None:
    logger.info("==================================================")
    logger.info("Multimodal Custom Models configuration")
    logger.info("  - Evaluation mode:          %s", cfg.evaluation_mode)
    logger.info("  - Model:                    %s", cfg.model.name)
    logger.info("  - Model checkpoint:         %s", cfg.model.checkpoint_path or "none")
    logger.info("  - Distillation enabled:     %s", cfg.distillation.enabled)
    if cfg.distillation.enabled:
        logger.info("  - Distillation mode:        %s", cfg.distillation.mode)
        logger.info("  - Audio teacher:            %s", cfg.distillation.audio_teacher.name)
        logger.info("  - Video teacher:            %s", cfg.distillation.video_teacher.name)
...
if __name__ == "__main__":
    try:
        ...
        logger.exception("Multimodal custom models run failed.")
        sys.exit(1)
